...
import { AdmissionsService, EvolutionsService } from '../../../core/services/clinical-records.service';
    ...
    <div class="min-h-screen bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
      ...
      @if (!isLoading() && errorMsg()) {
        <div class="rounded-xl border border-red-200 bg-red-50 p-8 text-center" role="alert">
          <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto mb-3 h-10 w-10 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4m0 4h.01M10.293 4.293a1 1 0 011.414 0l7 7a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7a1 1 0 010-1.414l7-7z"/>
          </svg>
          <p class="text-sm font-semibold text-red-700">Error al cargar la admisión</p>
          <p class="mt-1 text-xs text-red-600">{{ errorMsg() }}</p>
          <a routerLink="/admissions" class="mt-4 inline-block rounded-lg bg-red-600 px-4 py-2 text-xs font-medium text-white hover:bg-red-700">
            Volver a Admisiones
          </a>
        </div>
      }
      ...
      @if (!isLoading() && !errorMsg() && admission()) {
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              <div class="flex items-center gap-2">
                <span class="rounded-full bg-[#1e3a5f] px-2 py-0.5 text-xs font-semibold text-white">Admisión</span>
        ...
        <div class="space-y-6">
          ...
          <!-- ─── Patient & Date Info ─────────────────────────────────────── -->
          <section class="rounded-xl border border-slate-200 bg-white shadow-sm" aria-labelledby="section-info">
            <div class="border-b border-slate-100 px-6 py-4">
              <h2 id="section-info" class="flex items-center gap-2 text-base font-semibold text-[#1e3a5f]">
            ...
            <dl class="grid grid-cols-1 divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
              <div class="px-6 py-4">
                <dt class="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Paciente ID</dt>
                <dd class="font-mono text-sm text-slate-800">{{ admission()!.patient_id }}</dd>
              </div>
              <div class="px-6 py-4">
                <dt class="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Fecha de Admisión</dt>
            ...
            <ul class="divide-y divide-slate-50 px-6 py-4" aria-label="Motivos de consulta">
              @for (reason of admission()!.consult_reason; track $index) {
                <li class="flex items-start gap-3 py-2.5 first:pt-0 last:pb-0">
                  <span class="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[#1e3a5f]" aria-hidden="true"></span>
          ...
          <section class="rounded-xl border border-slate-200 bg-white shadow-sm" aria-labelledby="section-background">
            ...
            <ul class="divide-y divide-slate-50 px-6 py-4" aria-label="Antecedentes del paciente">
              @for (bg of admission()!.background; track $index) {
                <li class="flex items-start gap-3 py-2.5 first:pt-0 last:pb-0">
                  <span class="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-slate-400" aria-hidden="true"></span>
          ...
          <section class="rounded-xl border border-slate-200 bg-white shadow-sm" aria-labelledby="section-condition">
            ...
            <div class="px-6 py-5">
              <p class="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{{ admission()!.current_condition }}</p>
            </div>
          </section>
          ...
          <section class="rounded-xl border border-slate-200 bg-white shadow-sm" aria-labelledby="section-exam">
            ...
            <div class="px-6 py-5">
              <p class="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{{ admission()!.admission_exam }}</p>
            </div>
          </section>
          ...
          <section class="rounded-xl border border-slate-200 bg-white shadow-sm" aria-labelledby="section-diagnoses">
            <div class="border-b border-slate-100 px-6 py-4">
              <h2 id="section-diagnoses" class="flex items-center gap-2 text-base font-semibold text-[#1e3a5f]">
                ...
                Diagnósticos CIE-11
                <span class="ml-2 rounded-full bg-[#1e3a5f]/10 px-2 py-0.5 text-xs font-semibold text-[#1e3a5f]">
              ...
              @for (diagnosis of admission()!.admission_diagnosis; track diagnosis.code) {
                <div class="px-6 py-4">
                  <div class="flex items-start gap-3">
                    <span class="mt-0.5 flex-shrink-0 rounded bg-[#1e3a5f] px-2 py-0.5 font-mono text-xs font-bold text-white">
                      {{ diagnosis.code }}
                    </span>
          ...
          <!-- ─── Notas de Evolución ─────────────────────────────────────── -->
          <section class="rounded-xl border border-slate-200 bg-white shadow-sm" aria-labelledby="section-evolutions">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              <h2 id="section-evolutions" class="flex items-center gap-2 text-base font-semibold text-[#1e3a5f]">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
            ...
            @if (isLoadingEvolutions()) {
              <div class="flex items-center justify-center py-10" role="status" aria-live="polite">
                <svg class="mr-2 h-5 w-5 animate-spin text-[#1e3a5f]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
            ...
            @if (!isLoadingEvolutions()) {
              <div class="divide-y divide-slate-50">
                @for (evo of evolutions(); track evo.id) {
                  <article class="px-6 py-5" [attr.aria-label]="'Nota de evolución del ' + (evo.created_at | date:'dd/MM/yyyy')">
                    ...
                    <!-- View mode -->
                    @if (editingEvolutionId() !== evo.id) {
                      <p class="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{{ evo.description }}</p>
                    }
...
export class AdmissionDetailComponent implements OnInit, OnDestroy {
  ...
  private loadEvolutions(admissionId: string): void {
    ...
    this.evolutionsService.getByAdmission(admissionId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (list) => {
  ...
  saveEvolution(id: string): void {
    ...
    this.evolutionsService.update(id, { description: text }).pipe(takeUntil(this.destroy$)).subscribe({
      next: (updated) => {
        this.evolutions.update((list) =>
  ...
  deleteEvolution(id: string): void {
    ...
    this.evolutionsService.delete(id).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
